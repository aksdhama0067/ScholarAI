...
export async function POST(request: Request) {
  try {
    ...
    if (typeof body.text !== "string" || !body.text.trim()) {
      return new Response(JSON.stringify({ error: "Tell me what you would like to understand." }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    const fullText = await generateAIText({ system: "You are a patient teacher who does not talk down to learners.", prompt, maxOutputTokens, mode: "full" });
    if (!fullText) {
    ...
    return new Response(JSON.stringify({ error: 'I could not process that request.' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
