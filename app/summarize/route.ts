...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) && typeof rawText !== "string") {
      return NextResponse.json({ error: "Add a PDF or some text first." }, { status: 400 });
    }
    if (file instanceof File && file.size > maxFileBytes) {
      return NextResponse.json({ error: "Please use a document smaller than 10 MB." }, { status: 413 });
    }

    const source = typeof rawText === "string" ? rawText : file instanceof File ? await extractText(file) : "";
    const excerpt = cleanExcerpt(source);
    if (!excerpt) {
      return NextResponse.json({ error: "I could not find readable text in that file." }, { status: 422 });
    }
    ...
    const response = await generateAIText({
      system: instruction,
      prompt: `Summarize this study source:\n\n${excerpt}`,
      maxOutputTokens: 950,
    });
    ...
    const fallback: Summary = {
      ...demoSummary,
      title: file instanceof File ? file.name.replace(/\.[^/.]+$/, "") : "Your study material",
      overview: `Here is the learning thread I found: ${excerpt.slice(0, 240)}${excerpt.length > 240 ? "…" : ""}`,
    };
