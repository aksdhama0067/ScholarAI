...
import { BookOpen, CalendarDays, Compass, FileText, LayoutDashboard, Layers3, Sparkles, X } from "lucide-react";
    ...
    <div className="flex h-full flex-col p-4">
      <div className="mb-8 flex items-center justify-between px-2 pt-1">
        <button onClick={() => onChange("dashboard")} className="flex items-center gap-2 text-left" aria-label="Go to your desk">
          ...
          <span>
            <span className="block text-base font-bold tracking-tight">Scholar<span className="text-terra">AI</span></span>
            <span className="block font-mono text-[9px] tracking-[.14em] text-ink/45">STUDY STUDIO</span>
          </span>
        </button>
...
export function Sidebar({ active, onChange, open, onClose }: Props) {
  return (
    <>
      <aside className="hidden min-h-screen w-[250px] shrink-0 border-r border-line bg-white/55 lg:block">
